...
def _buchungen_ohne_agenda(
    ...
    for r in pdf_rows:
        skip = bool(r.get("_skip_buchung_mapping"))
        row: dict[str, Any] = {
            "Umsatz Euro": r["Umsatz Euro"],
            "BU Gkto": r.get("BU Gkto") or "",
            "Beleg 1": beleg,
            "Datum": r["Datum"],
            "KOST 1": cfg["KOST"],
            "Bank": cfg["BANK_KONTO"],
            "Buchungstext": (
                r["Buchungstext"]
                if skip
                else kuerze_stripe_text(r["Buchungstext"], r["Datum"])
            ),
        }
        if skip:
...
def _log_export_summary(
    buchungen_rows: list[dict[str, Any]],
    allopay_rows: list[dict[str, Any]],
) -> None:
...
def run_etl(config: AsiaEtlConfig, *, excel_titel: str | None = None) -> None:
    ...
    # Tilgung/Zins aus Banktext (vor Agenda/Mapping), Beleg = Monatsnummer
    beleg = beleg_month_from_pdf(cfg["PDF_FILE"], pdf_rows)
    pdf_rows = expand_sparkasse_darlehen_rows(pdf_rows)
    ...
    if df_agenda is not None:
        buchungen_rows = merge_mit_agenda_und_split(
            pdf_rows, df_agenda, cfg, beleg_1=beleg
        )
    else:
    ...
    if suf in (".sqlite", ".db"):
        exportiere_sqlite(buchungen_rows, allopay_rows, str(out_path), cfg)
        logger.info("Nur SQLite – kein Excel, kein Final-Blatt.")
    elif suf == ".sql":
        exportiere_sql_skript(buchungen_rows, allopay_rows, str(out_path), cfg)
    ...
    if sql_extra:
        ...
        try:
            Path(sql_extra).parent.mkdir(parents=True, exist_ok=True)
            exportiere_sqlite(buchungen_rows, allopay_rows, sql_extra, cfg)
        except Exception as exc:
            logger.warning("SQLite-Nebenexport übersprungen (%s): %s", sql_extra, exc)

    _log_export_summary(buchungen_rows, allopay_rows)
